...
<script>
  ...
  try {
    // Firebase-ის ინიციალიზაცია (თუ უკვე ინიციალიზებული არ არის)
    if (!firebase.apps.length) {
        firebase.initializeApp(firebaseConfig);
    }
    const db = firebase.firestore();
    const MEDICATION_COLLECTION = 'medications';
    ...
    async function saveMedication(){
      ...
      try {
        ...
        alert('შეცდომა მონაცემების შენახვისას. შეამოწმეთ Firebase Console და Rules.');
      }
    ...
    window.deleteMed = deleteMed;
      ...
      document.getElementById('list').innerHTML = `<p style="text-align:center;color:red;padding:18px">კრიტიკული შეცდომა Firebase-ის ინიციალიზაციისას: ${e.message}. შეამოწმეთ, არის თუ არა SDK-ები (firebase-app, firebase-firestore) ჩატვირთული HEAD-ში.</p>`;
